...
/* ==========================================================================
   TEAM MEMBERS & COLLABORATORS CREDITS LOGIC
   ========================================================================== */
...
function initCollaborators() {
  ...
  filterBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      filterBtns.forEach(b => b.classList.remove("active"));
      btn.classList.add("active");
      currentCollabFilter = btn.dataset.filter || "all";
      renderCollaborators(currentCollabFilter);
    });
  });

  updateCollabFilterCounts();
  renderCollaborators("all");
}

function updateCollabFilterCounts() {
  ...
  const counts = {
    all: compData.length,
    datastorm: compData.filter(c => c.filterTags && c.filterTags.includes("datastorm")).length,
    nexushacks: compData.filter(c => c.filterTags && c.filterTags.includes("nexushacks")).length,
    modelx: compData.filter(c => c.filterTags && c.filterTags.includes("modelx")).length,
    robotics: compData.filter(c => c.filterTags && c.filterTags.includes("robotics")).length,
    hackathons: compData.filter(c => c.filterTags && c.filterTags.includes("hackathons")).length
  };

  for (const [key, val] of Object.entries(counts)) {
    const el = document.getElementById(`count-collab-${key}`);
    if (el) el.textContent = val;
  }
}

function renderCollaborators(filter = "all") {
  const grid = document.getElementById("collaborators-grid");
  if (!grid) return;
  ...
  if (filter === "all") {
    ...
    list = compData.filter(item => item.filterTags && item.filterTags.includes(filter));
  }
  ...
  if (list.length === 0) {
    grid.innerHTML = `
      <div style="grid-column: 1 / -1; text-align: center; padding: 3rem; background: var(--bg-card); border-radius: var(--radius-md); border: 1px dashed var(--border-subtle);">
        <i class="fas fa-trophy" style="font-size: 2rem; color: var(--text-muted); margin-bottom: 0.75rem;"></i>
        <p style="color: var(--text-primary); font-weight: 600;">No competitions found in this category.</p>
        <p style="color: var(--text-secondary); font-size: 0.85rem;">Select "All Competitions" to view all events and collaborators.</p>
      </div>
    `;
    return;
  }
  ...
  list.forEach((comp, idx) => {
    // Resolve members for this competition
    ...
    card.id = `comp-collab-card-${comp.filterId || idx}`;
...
/**
 * Global Professional Team Credits & Portfolio Modal
 */
window.openTeamCreditsModal = function(compId) {
  ...
  // Animation trigger & body scroll lock
